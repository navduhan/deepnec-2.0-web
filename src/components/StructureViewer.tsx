...
export default function StructureViewer({ pdb, sequence='', sampleId='structure' }: { pdb: string; sequence?:string; sampleId?:string }) {
  ...
  useEffect(()=>{if(!ready||!componentRef.current)return;componentRef.current.removeAllRepresentations();componentRef.current.addRepresentation(representation,{colorScheme:color});if(selection)componentRef.current.addRepresentation('ball+stick',{sele:selection,color:'#ff00ff',aspectRatio:2});},[ready,representation,color,selection]);
