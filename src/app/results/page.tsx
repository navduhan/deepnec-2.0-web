...
export default function ResultsPage(){
  ...
  const files=useMemo(()=>Object.keys(data?.results||{}).sort((a,b)=>rank(a)-rank(b)),[data]);const rows=data?.results?.[active]||[];const columns=Object.keys(rows[0]||{}).filter(h=>!(active.includes('Phase_3')&&h==='Prediction'&&Object.keys(rows[0]||{}).some(k=>k.replaceAll('_','').toLowerCase()==='predictedpathway')));const expiry=data?.updatedAt?expiresAt(data.updatedAt):null;
  function openStructure(kind:'secondary'|'tertiary',sampleId:string){if(!bookmark)return;const fragment=new URLSearchParams({job:bookmark.jobId,token:bookmark.jobToken,sample:sampleId,kind,phase:active});window.location.assign(withBasePath('/structure')+'#'+fragment);}
  ...
  return <div className="space-y-6 py-5 sm:py-8">
    <header className="rounded-[1.75rem] bg-[var(--navy)] px-6 py-8 text-white"><p className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em] text-[#9fccef]"><CheckCircle2 className="h-4 w-4"/>Prediction complete</p><h1 className="mt-2 font-display text-4xl font-semibold">DeepNEC results</h1><p className="mt-2 text-sm text-slate-300">Review predictions, annotations, motifs, and protein structures.</p></header>
