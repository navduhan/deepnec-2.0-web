...
import { predictTertiary } from '@/lib/structure';
...
export async function POST(req: NextRequest) {
  try {
    ...
    enforceRateLimit('tertiary-structure', clientIp(req), 5, 24 * 60 * 60 * 1000);
    ...
    return NextResponse.json({ sampleId, sequence, ...(await predictTertiary(sequence)) });
  } catch (error: unknown) {
