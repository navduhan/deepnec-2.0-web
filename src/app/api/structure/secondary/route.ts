...
export async function POST(req: NextRequest) {
  try {
    ...
    enforceRateLimit('secondary-structure', clientIp(req), 10, 60 * 60 * 1000);
    ...
    return NextResponse.json({ sampleId, sequenceLength: sequence.length, ...(await predictSecondary(sequence)) });
  } catch (error: unknown) {
