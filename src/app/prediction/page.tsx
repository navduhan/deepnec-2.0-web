...
export default function PredictionPage() {
  ...
  async function fetchAccessions() {
    ...
    try {
      ...
      setSequenceType('prot');setSequence(data.fasta); setMode('paste'); setDemoLoaded(false);
  ...
  async function submit() {
    ...
    try {
      const response = await fetch(withBasePath('/api/predict'), { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({sequence,sequenceType,level,pathway:level==='Phase4'?pathway:'all',model:'final',turnstileToken:turnstile}) });
      const job = await parseResponse(response);
  ...
  return <div className="space-y-7 py-4 sm:py-8">
    <header className="rounded-[1.75rem] border border-[#d7e5f0] bg-gradient-to-r from-[#f3f8fc] to-[#fff7fa] px-6 py-8 sm:px-9"><p className="eyebrow">DeepNEC prediction server</p><h1 className="mt-2 font-display text-3xl font-semibold text-[var(--navy)] sm:text-4xl">Predict sequence function</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">Provide protein or nucleotide sequences and choose the level of functional detail you need.</p></header>
    ...
    <div className="grid gap-7 lg:grid-cols-[1.2fr_.8fr]">
      <section className="surface p-5 sm:p-7"><div className="flex items-start justify-between"><div><p className="eyebrow">Step 1</p><h2 className="mt-1 font-display text-2xl font-semibold text-[var(--navy)]">Sequence input</h2></div><button type="button" onClick={clear} className="btn-secondary"><RotateCcw className="h-3.5 w-3.5"/> Clear</button></div>
        <label className="mt-5 block text-sm font-semibold">Sequence type<select className="field mt-2" value={sequenceType} onChange={e=>setSequenceType(e.target.value as 'prot'|'nucl')}><option value="prot">Protein</option><option value="nucl">Nucleotide — translate with TransDecoder</option></select></label>{sequenceType==='nucl'&&<p className="mt-2 text-sm">TransDecoder extracts open reading frames. Predictions and structures refer to the translated proteins.</p>}<p className="mt-3 text-sm">For large protein datasets or repeated analyses, use the <a className="font-semibold underline" href={withBasePath('/download')}>standalone tool</a> on your computer or cluster.</p><div className="mt-5 grid grid-cols-3 gap-1 rounded-xl bg-[var(--ice)] p-1">{(['paste','upload','accession'] as const).map((item)=><button key={item} type="button" onClick={()=>setMode(item)} className={`rounded-lg px-3 py-2 text-xs font-bold capitalize ${mode===item?'bg-white text-[var(--navy)] shadow-sm':'text-slate-500'}`}>{item}</button>)}</div>
        ...
        <div className={`${mode==='paste'?'mt-5':'mt-4'}`}><div className="mb-2 flex items-center justify-between"><label htmlFor="deepnec-fasta" className="text-sm font-bold text-[var(--navy)]">FASTA sequence</label><button type="button" onClick={loadExample} className="btn-secondary">Load example</button></div><textarea id="deepnec-fasta" rows={12} className="field resize-y font-mono text-xs leading-6" value={sequence} onChange={(e)=>{setSequence(e.target.value);setDemoLoaded(false)}} placeholder=">protein_id&#10;MSEQUENCE…"/><p className="mt-2 text-xs text-slate-500">{count ? `${count} protein record${count===1?'':'s'} detected` : sequenceType==='nucl'?'Use nucleotide FASTA (A, C, G, T, U, N).':'Use standard amino-acid FASTA. X residues are accepted and removed before prediction.'}</p></div>
      </section>
