...
const stages = [
  ...
  ['04', 'EC number', '21 deployable terminal labels'],
];
...
export default function Home() {
  return <div className="space-y-16 pb-12 pt-4 sm:pt-8">
    ...
    <section id="workflow" className="scroll-mt-28">
      <p className="eyebrow">Prediction architecture</p><h2 className="mt-2 font-display text-3xl font-semibold text-[var(--navy)] sm:text-4xl">A route-aware biological workflow</h2>
    ...
    <section className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
      <article className="surface p-7 sm:p-9"><p className="eyebrow">Built for reproducible analysis</p><h2 className="mt-3 font-display text-3xl font-semibold text-[var(--navy)]">Final frozen ESM-2 models, deployed together.</h2><div className="mt-6 grid gap-4 sm:grid-cols-2"><p className="flex gap-3 text-sm leading-6 text-slate-600"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-[var(--blue)]" />Eight final learned models with fixed class orders and verified model hashes.</p><p className="flex gap-3 text-sm leading-6 text-slate-600"><ShieldCheck className="mt-1 h-4 w-4 shrink-0 text-[var(--blue)]" />Private result links and HPC execution isolate large inference workloads from the web server.</p></div></article>
      <aside className="rounded-[1.5rem] bg-[var(--navy)] p-7 text-white sm:p-9"><BookOpen className="h-6 w-6 text-[#8fc6ef]" /><h2 className="mt-5 font-display text-3xl font-semibold">Ready to annotate?</h2><p className="mt-3 text-sm leading-6 text-slate-300">Paste FASTA, upload a protein file, or retrieve reviewed accessions before submission.</p><Link href="/prediction" className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-[#9fd0f4] hover:text-white">Open prediction server <ArrowRight className="h-4 w-4" /></Link></aside>
    </section>
  </div>;
}
