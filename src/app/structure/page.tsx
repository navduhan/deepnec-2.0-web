...
export default function StructurePage(){
  ...
  const load=useCallback(async()=>{setBusy(true);setError('');try{const value=await fetchStructure(info.kind,info.job,info.sample,info.token);setData(value);}catch(e){setError(e instanceof Error?e.message:'Structure unavailable.');}finally{setBusy(false);}},[info]);
  useEffect(()=>{if(!info.job)return;let cancelled=false;void Promise.resolve().then(()=>{if(cancelled)return;try{const saved=JSON.parse(sessionStorage.getItem('deepnec_structure_view')||'null');if(saved?.job===info.job&&saved?.sample===info.sample&&saved?.kind===info.kind){setData(saved.data);return;}}catch{/* Fall back to the server cache. */}void load();});return()=>{cancelled=true;};},[info,load]);
  const back=withBasePath('/results')+'#'+new URLSearchParams({job:info.job,token:info.token,phase:info.phase});
  return <main className="space-y-5 py-8"><a href={back} className="btn-secondary">← Back to results table</a><h1 className="text-3xl font-semibold">{info.kind==='secondary'?'Secondary structure':'3D structure'}</h1><p className="break-all">{info.sample}</p>{!data&&!error&&<p role="status">Structure prediction in progress. This page will update when it is ready.</p>}{error&&<button className="btn-secondary" disabled={busy||!info.job} onClick={()=>void load()}>Retry loading</button>}{error&&<p role="alert" className="text-[var(--danger)]">{error}</p>}{data&&(info.kind==='secondary'?<><button className="btn-secondary" onClick={()=>{const a=document.createElement('a');const url=URL.createObjectURL(new Blob([`>${data.sampleId}\nAA: ${data.amino_acids}\nPred: ${data.prediction}\nConf: ${data.confidence}\n`],{type:'text/plain'}));a.href=url;a.download='secondary-structure.txt';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}}>Download prediction</button><SecondaryStructureViewer data={data}/></>:<><p>{data.method}</p><Viewer pdb={data.pdb} sequence={data.sequence} sampleId={data.sampleId}/></>)}</main>;
}
